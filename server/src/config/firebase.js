...
const initFirebase = () => {
  ...
  if (!serviceAccountKey) {
    console.warn(
      "WARNING: FIREBASE_SERVICE_ACCOUNT_KEY is not defined in .env. Real Firebase Auth will fail; please use mock- tokens for local development.",
    );
    return;
  }
  ...
  try {
    ...
    if (serviceAccountKey.trim().startsWith("{")) {
      ...
      try {
        ...
        throw new Error(
          "Could not read or parse Firebase service account key: " + e.message,
        );
      }
    }
